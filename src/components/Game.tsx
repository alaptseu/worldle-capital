...
import { getCountryCapital, sanitizeCapital } from "../domain/countries";
...
import { countriesCapitals } from "../domain/countries.capitals.position";
...
export function Game({ settingsData, updateSettings }: GameProps) {
  ...
  const countryName = useMemo(
    () => (country ? getCountryCapital(i18n.resolvedLanguage, country) : ""),
  ...
  const handleSubmit = useCallback(
    (e: React.FormEvent<HTMLFormElement>) => {
      ...
      e.preventDefault();
      const guessedCountry = countriesCapitals.find(
        (country) =>
          sanitizeCapital(getCountryCapital(i18n.resolvedLanguage, country)) ===
          sanitizeCapital(currentGuess)
      );
  ...
  useEffect(() => {
    ...
    if (
      ...
      toastId = toast.info(
        getCountryCapital(i18n.resolvedLanguage, country).toUpperCase(),
        {
          autoClose: false,
          delay: 2000,
        }
